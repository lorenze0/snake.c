#include<stdio.h>
#include<stdlib.h>		//生成随机数 ，random也可以，但是不是标准函数，不能在vc/vs等编译器通过 。 
						//RAND_MAX 是 <stdlib.h> 头文件中的一个宏，它用来指明 rand() 所能返回的随机数的最大值。C语言标准并没有规定 RAND_MAX 的具体数值，只是规定它的值至少为 32767
#include<math.h>
#include<conio.h>		//用于非阻塞式监听，特用于win
#include<time.h>		//用于生成随机数中的种子 
#include<windows.h>		//SetConsoleTextAttribute 函数改变文字和背景颜色,特用于win 

//MAXWIDTH、MAXHEIGHT、INITLEN 用字符来记
#define MAXWIDTH (30) 
#define MAXHEIGHT MAXWIDTH
#define INITLEN (3)		//这里记录的是贪吃蛇的初始长度，记录为3 
...
void init()
{
	...
	for(x = 0; x < MAXHEIGHT; x ++ )
	{
		for(y = 0 ; y < MAXWIDTH; y ++ )
		{
			...
			{
				index = (x - 1) * (MAXWIDTH - 2) + (y - 1); //就这里导致了跳过了两个格子 
				snakeMap[index].x = x;
				snakeMap[index].y = y;
